...
int main() {                            // este programa deve ser rodado com 'sudo'
   ...
   pwmSetClock(569);                     // fornece uma frequencia de 261.6Hz (max=4096
    ...
    for (repeat = 0; repeat < 2; repeat++){  //Repetição do alarme
     for(ciclos = 0; ciclos < 5; ciclos++){  // variando o duty cycle
        if(ciclos == 2){  
            usleep(500000) // pausa de 0.5seg no alarme
        }
        else {
            for(dc = 0; dc < 129; dc++){
     ...
     return 0;    
     }                        // a saida PWM permanece ligada apos o termino do programa
  }
